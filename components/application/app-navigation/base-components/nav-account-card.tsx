...
export type NavAccountType = {
    ...
    /** Avatar image URL. */
    avatar?: string;
    /** Initials shown when an account has no profile image. */
    initials?: string;
...
export const NavAccountMenu = ({
    ...
    onOpenSettings,
    ...dialogProps
}: AriaDialogProps & { className?: string; accounts?: NavAccountType[]; selectedAccountId?: string; onOpenSettings?: () => void }) => {
    ...
    return (
        <AriaDialog
            {...dialogProps}
            ref={dialogRef}
            className={cx("w-66 rounded-xl bg-[#f7f9fb] shadow-lg ring-1 ring-[#d6e1eb] outline-hidden", className)}
        >
            {({ close }) => (
                <>
                    <div className="rounded-xl bg-white ring-1 ring-[#dce5ed]">
                        ...
                        <div className="flex flex-col gap-0.5 border-t border-[#dce5ed] py-1.5">
                            <div className="px-3 pt-1.5 pb-1 text-xs font-semibold text-[#526175]">Switch account</div>
                        ...
                        <div className="flex flex-col gap-2 px-2 pt-0.5 pb-2">
                            <Button iconLeading={Plus} color="secondary" size="sm" className="border border-[#d0d9e3]">
                                Add account
                            </Button>
                        </div>
                    </div>
    ...
    return (
        <button {...buttonProps} className={cx("group/item w-full cursor-pointer px-1.5 focus:outline-hidden", buttonProps.className)}>
            ...
            >
                ...
                {shortcut && (
                    <kbd className="flex rounded px-1 py-px font-body text-xs font-medium text-[#66758a] ring-1 ring-[#d0d9e3] ring-inset">{shortcut}</kbd>
                )}
            </div>
        </button>
    );
...
export const NavAccountCard = ({
    ...
    popoverPlacement?: ComponentProps<typeof AriaPopover>["placement"];
    ...
    return (
        <div ref={triggerRef} className="relative flex items-center gap-3 rounded-xl bg-white p-3 ring-1 ring-[#d0d9e3] ring-inset">
            <AvatarLabelGroup
                size="md"
            ...
            <AriaDialogTrigger>
                ...
                >
                    <NavAccountMenu selectedAccountId={selectedAccountId} accounts={items} onOpenSettings={onOpenSettings} />
                </AriaPopover>
            </AriaDialogTrigger>
        </div>
    );
