...
export const RadioButtonBase = ({ className, isFocusVisible, isSelected, isDisabled, size = "sm" }: RadioButtonBaseProps) => {
    return (
        <div
            className={cx(
                "flex size-4 shrink-0 cursor-pointer appearance-none items-center justify-center rounded-full bg-white ring-1 ring-[#d0d5dd] ring-inset",
                size === "md" && "size-5",
                isSelected && "bg-[#236a7c] ring-[#236a7c]",
        ...
        >
            <div className={cx("size-1.5 rounded-full bg-white opacity-0 transition-inherit-all", size === "md" && "size-2", isSelected && "opacity-100")} />
        </div>
    );
};
