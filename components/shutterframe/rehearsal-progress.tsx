...
export function RehearsalProgress({ stages, compact = false }: { stages: RehearsalStage[]; compact?: boolean }) {
  ...
  return <section className={`dashboard-card overflow-hidden ${compact ? "px-5 py-5" : "px-6 py-6 sm:px-10"}`} aria-labelledby="progress-heading">
    ...
    <div className={`relative ${compact ? "mx-6 mt-7 h-[118px] sm:mx-8" : "mx-10 mt-8 h-[190px] sm:mx-14"}`} role="progressbar" aria-label="Rehearsal progress" aria-valuemin={0} aria-valuemax={stages.length} aria-valuenow={reachedStages.size}>
